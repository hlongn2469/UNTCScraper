...
class UNTCScraper(object):
    ...
    def scrape1(self):
        ...
        self.df = parse_tr_list(self._parse1(content))

        for pageno in range(2, self.endpage+1):
            ## sleep < 10s
            time.sleep(random.random()*10)
            self.df = self.df.append(parse_tr_list(self.scrape_page1(pageno)), ignore_index=True)

    def scrape_page1(self, pageno):
        self.br.select_form('aspnetForm')
        ctl = self.br.form.find_control('__EVENTTARGET')
        ctl.readonly = False
