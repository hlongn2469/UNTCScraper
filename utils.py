...
from extension import find_Par, find_Sterms
...
def parse_tr_list(tr_list):
    ...
    return pd.DataFrame([parse_tr(tr) for tr in l],
                        columns = ('title','subj_terms', 'participant(s)', 'reg_num', 'reg_date', 't_type', 'conclusion_date', 'volume'))
...
def parse_tr(tr):
    ...
    for i, td in enumerate(td_list):
        ...
        if i == 0:
            ...
            l.append(find_Sterms(url,l))
            l.append(find_Par(url,l))
    return l
